...
def data_plot(init, predSet, testSet):
    if predSet is not None and testSet is not None:
        save_path = './plots/prediction_in_'+init.model+'.pdf'
        fig = plt.figure()
...
def evaluate_model(init, data, predict):
    ...
    if predict is not None:
        ...
        metrics_df.to_csv('./results/test/Metric_'+init.model+'_'+metric_name+'.csv', encoding='utf-8', index=False)
    else:
        exit(1)
...
def exponentially_weighted_moving_average(data):
    ...
    while (count < len(df)):
        ...
        count+=5
    return store_throughput, np.array(EWMA)
...
def harmonic_mean(data):
    ...
    while(k < len(store_throughput)):
        ...
        k+=1
    return np.array(store_throughput), np.array(arr_array)
    ...
    if init.data is not None and init.model is not None:        
        ...
        if init.approach == 'DL': 
            ...
            if init.approach == 'MA':
                ...
                if init.model == 'SMA':
                    real_data, sma_data = simple_moving_average(Data)
                    # print('real', 'SMA', real_data.shape, sma_data.shape)
                    # evaluate_model(init, data_of_sma, pred_data)
                    evaluate_model(init, real_data, sma_data)
                    
                # Harmonic Mean [HM] 
                if init.model == 'HM':
                    real_data, hm_data = harmonic_mean(Data)
                    evaluate_model(init, real_data, hm_data)
                
                # Exponentially Weighted Moving Average [EWMA]
                elif init.model == 'EWMA':
                    real_data, ewma_data = exponentially_weighted_moving_average(Data)
                    evaluate_model(init, real_data, ewma_data)
            else:
                raise TypeError("Model is not defined")
